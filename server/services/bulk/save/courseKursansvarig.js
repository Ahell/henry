export function buildCourseKursansvarigToInsert({
  courseKursansvarigPayload,
  coursesPayload,
  remapCourseId,
  remapTeacherId,
} = {}) {
  ...
  if (Array.isArray(courseKursansvarigPayload)) {
    courseKursansvarigPayload.forEach((row) =>
  ...
  if (Array.isArray(coursesPayload)) {
    coursesPayload.forEach((c) =>
      pushRow(c?.course_id, c?.kursansvarig_teacher_id)
    );
  }
